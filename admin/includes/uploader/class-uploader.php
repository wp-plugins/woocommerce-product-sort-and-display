...
class WC_PSAD_Uploader extends WC_PSAD_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'wc_psad' ).'" rel="'.$post_id.'" /> '.$description;
	...
	public function change_button_text( $translation, $original ) {
	    ...
	    if ( $original == 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'wc_psad' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'wc_psad' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'wc_psad' ), $tabs['gallery'] ); }
		return $tabs;
	}
