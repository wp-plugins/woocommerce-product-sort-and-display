...
class WC_PSAD_Admin_Interface extends WC_PSAD_Admin_UI
{
	...
	public function admin_forms( $options, $form_key, $option_name = '', $form_messages = array() ) {
			...
			switch( $value['type'] ) {
				...
				case 'upload':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
                        	<?php echo $description; ?>
                        	<?php echo $wc_psad_uploader->upload_input( $name_attribute, $id_attribute, $option_value, $value['default'], esc_html( $value['name'] ), $class, esc_attr( $value['css'] ) , '' );?>
						</td>
					</tr><?php
		...
		<?php do_action( $this->plugin_name . '-' . trim( $form_key ) . '_settings_end' ); ?>
            <p class="submit">
                    <input type="submit" value="<?php _e('Save changes', 'wc_psad'); ?>" class="button-primary" name="bt_save_settings" />
